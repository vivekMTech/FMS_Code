...
class WriteCountedString(WriteRandomString):
    def __init__(self):
        WriteRandomString.__init__(self)
        # super().__init__()
        # self.const_string = 'MARUTI'  # take constant keyword as 'MARUTI'
...
async def main():
    write_random_obj = WriteRandomString()  # WriteRandomString object for accessing coroutines
    write_count_obj = WriteCountedString()  # WriteCountedString object for accessing coroutines
    while True:
        write_file1 = asyncio.create_task(
            write_random_obj.write_random_strings("file1.txt"))  # call write_random_strings() for file1
        write_file2 = asyncio.create_task(
            write_random_obj.write_random_strings("file2.txt"))  # call write_random_strings() for file2
        write_count_log = asyncio.create_task(write_count_obj.write_counts("file1.txt", "file2.txt"))  # call write in count.log
